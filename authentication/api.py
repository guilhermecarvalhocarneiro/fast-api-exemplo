...
def login_access_token(db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends()) -> Any:
    ...
    """
    user = cruds.user.authenticate(db, username=form_data.username, password=form_data.password)
    if not user:
...
def update_user_me(*, db: Session = Depends(get_db), first_name: str = Body(None), last_name: str = Body(None),
                   email: EmailStr = Body(None),
                   current_user: schemas.User = Depends(security.get_current_active_user)) -> Any:
    """
    Update own user.
    """
...
router_permission = APIRouter(prefix="/permisions", tags=["permisions"],
...
def read_permissions(db: Session = Depends(get_db), skip: int = 0, limit: int = 25) -> Any:
    """
...
def create_permission(*, db: Session = Depends(get_db), permission_in: schemas.PermissionCreate) -> Any:
    """
...
def read_permission_by_id(permission_id: int, db: Session = Depends(get_db)) -> Any:
    ...
    if not permission:
        raise HTTPException(status_code=404, detail="The permission does not exist in the system")
    return permission


@router_permission.put("/{permission_id}", response_model=schemas.Permission)
def update_permission(*, db: Session = Depends(get_db), permission_id: int,
                      permission_in: schemas.PermissionUpdate) -> Any:
    """
    Update a permission.
    """
    permission = cruds.permission.get(db, id=permission_id)
...
router_group = APIRouter(prefix="/groups", tags=["groups"], dependencies=[Depends(security.get_current_active_user)])
...
def create_group(*, db: Session = Depends(get_db), group_in: schemas.GroupCreate) -> Any:
    """
    Create new group.
    """
...
def read_group_by_id(group_id: int, db: Session = Depends(get_db)) -> Any:
    ...
    if not group:
        raise HTTPException(status_code=404, detail="The group does not exist in the system")
    return group


@router_group.put("/{group_id}", response_model=schemas.Group)
def update_group(*, db: Session = Depends(get_db), group_id: int, group_in: schemas.GroupUpdate) -> Any:
    """
    Update a group.
    """
    group = cruds.group.get(db, id=group_id)
