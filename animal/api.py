...
@router_especie.get(
    "/",
    response_model=List[schemas.Especie],
...
def create_especie(
    *,
    db: Session = Depends(get_db),
    especie_in: schemas.EspecieCreate,
) -> Any:
    """
    Create new especie.
    """
...
def read_especie_by_id(
    ...
    if not especie:
        raise HTTPException(
            status_code=404,
            detail="Esse especie não existe no sistema",
        )
    return especie
...
def update_especie(
    *,
    db: Session = Depends(get_db),
    especie_id: str,
    especie_in: schemas.EspecieUpdate
) -> Any:
    """
    Update a especie.
    """
...
@router_especie.delete("/{id}", response_model=schemas.Especie, dependencies=[
                       Depends(security.has_permission("animal.delete_especie"))])
def delete_especie(
    *,
    db: Session = Depends(get_db),
    id: str
...
def read_animal_by_id(
    ...
    if not animal:
        raise HTTPException(
            status_code=404,
            detail="Esse animal não existe no sistema",
        )
    return animal
...
@router_animal.delete("/{id}", response_model=schemas.Animal, dependencies=[
                      Depends(security.has_permission("animal.delete_animal"))])
def delete_animal(
    *,
    db: Session = Depends(get_db),
    id: str
