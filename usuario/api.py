...
def read_usuario_by_id(
    ...
    if not usuario:
        raise HTTPException(
            status_code=404,
            detail="Esse usuario não existe no sistema",
        )
    return usuario


@router_usuario.put("/{usuario_id}", response_model=schemas.Usuario,
...
@router_usuario.delete("/{id}", response_model=schemas.Usuario, dependencies=[
                       Depends(security.has_permission("usuario.delete_usuario"))])
def delete_usuario(
    *,
    db: Session = Depends(get_db),
    id: str
...
@router_cliente.post("/", response_model=schemas.Cliente,
                     dependencies=[Depends(security.has_permission("usuario.add_cliente"))])
def create_cliente(
    *,
    db: Session = Depends(get_db),
...
def read_cliente_by_id(
    ...
    if not cliente:
        raise HTTPException(
            status_code=404,
            detail="Esse cliente não existe no sistema",
        )
    return cliente


@router_cliente.put("/{cliente_id}", response_model=schemas.Cliente,
...
@router_cliente.delete("/{id}", response_model=schemas.Cliente, dependencies=[
                       Depends(security.has_permission("usuario.delete_cliente"))])
def delete_cliente(
    *,
    db: Session = Depends(get_db),
    id: str
...
@router_profissao.get(
    "/",
    response_model=List[schemas.Profissao],
...
def create_profissao(
    *,
    db: Session = Depends(get_db),
    profissao_in: schemas.ProfissaoCreate,
) -> Any:
    """
...
def read_profissao_by_id(
    ...
    if not profissao:
        raise HTTPException(
            status_code=404,
            detail="Esse profissao não existe no sistema",
        )
    return profissao
...
def update_profissao(
    *,
    db: Session = Depends(get_db),
    profissao_id: str,
    profissao_in: schemas.ProfissaoUpdate
) -> Any:
    """
    Update a profissao.
    """
    ...
    profissao = cruds.profissao.update(
        db, db_obj=profissao, obj_in=profissao_in)
    return profissao


@router_profissao.delete("/{id}", response_model=schemas.Profissao, dependencies=[
                         Depends(security.has_permission("usuario.delete_profissao"))])
def delete_profissao(
    *,
    db: Session = Depends(get_db),
    id: str
) -> Any:
...
@router_profissional.get(
    "/",
    response_model=List[schemas.Profissional],
    dependencies=[Depends(security.has_permission("usuario.view_profissional"))]
)
def read_profissionals(
    db: Session = Depends(get_db),
    skip: int = 0,
    limit: int = 25
) -> Any:
...
@router_profissional.post("/", response_model=schemas.Profissional, dependencies=[
                          Depends(security.has_permission("usuario.add_profissional"))])
def create_profissional(
    *,
    db: Session = Depends(get_db),
    profissional_in: schemas.ProfissionalCreate,
) -> Any:
    """
...
@router_profissional.get(
    "/{profissional_id}",
    response_model=schemas.Profissional,
    dependencies=[
        Depends(
            security.has_permission("usuario.view_profissional"))])
def read_profissional_by_id(
    ...
    if not profissional:
        raise HTTPException(
            status_code=404,
            detail="Esse profissional não existe no sistema",
        )
    return profissional


@router_profissional.put(
    "/{profissional_id}",
    response_model=schemas.Profissional,
    dependencies=[
        Depends(
            security.has_permission("usuario.change_profissional"))])
def update_profissional(
    *,
    db: Session = Depends(get_db),
    profissional_id: str,
    profissional_in: schemas.ProfissionalUpdate
) -> Any:
    """
    ...
    profissional = cruds.profissional.update(
        db, db_obj=profissional, obj_in=profissional_in)
    return profissional


@router_profissional.delete(
    "/{id}",
    response_model=schemas.Profissional,
    dependencies=[
        Depends(
            security.has_permission("usuario.delete_profissional"))])
...
router_profissionalservico = APIRouter(
    prefix="/profissionalservico",
    tags=['profissionalservico'],
    dependencies=[Depends(security.get_current_active_user)]
)


@router_profissionalservico.get(
    "/",
    response_model=List[schemas.ProfissionalServico],
    dependencies=[Depends(security.has_permission("usuario.view_profissionalservico"))]
)
def read_profissionalservicos(
    db: Session = Depends(get_db),
    skip: int = 0,
    limit: int = 25
) -> Any:
    ...
    """
    profissionalservicos = cruds.profissionalservico.get_multi(
        db, skip=skip, limit=limit)
    return profissionalservicos


@router_profissionalservico.post(
    "/",
    response_model=schemas.ProfissionalServico,
    dependencies=[
        Depends(
            security.has_permission("usuario.add_profissionalservico"))])
def create_profissionalservico(
    *,
    db: Session = Depends(get_db),
    profissionalservico_in: schemas.ProfissionalServicoCreate,
) -> Any:
    """
    Create new profissionalservico.
    """
    profissionalservico = cruds.profissionalservico.create(
        db, obj_in=profissionalservico_in)
...
@router_profissionalservico.get(
    ...
    dependencies=[
        Depends(
            security.has_permission("usuario.view_profissionalservico"))])
def read_profissionalservico_by_id(
    profissionalservico_id: str,
    db: Session = Depends(get_db)
) -> Any:
    """
    Get a specific profissionalservico by id.
    """
    profissionalservico = cruds.profissionalservico.get(
        db, id=profissionalservico_id)
    if not profissionalservico:
        raise HTTPException(
            status_code=404,
            detail="Esse profissionalservico não existe no sistema",
        )
    return profissionalservico


@router_profissionalservico.put(
    "/{profissionalservico_id}",
    response_model=schemas.ProfissionalServico,
    dependencies=[
        Depends(
            security.has_permission("usuario.change_profissionalservico"))])
def update_profissionalservico(
    *,
    db: Session = Depends(get_db),
    profissionalservico_id: str,
    profissionalservico_in: schemas.ProfissionalServicoUpdate
) -> Any:
    """
    ...
    profissionalservico = cruds.profissionalservico.update(
        db, db_obj=profissionalservico, obj_in=profissionalservico_in)
    return profissionalservico


@router_profissionalservico.delete(
    "/{id}",
    response_model=schemas.ProfissionalServico,
    dependencies=[
        Depends(
            security.has_permission("usuario.delete_profissionalservico"))])
def delete_profissionalservico(
    *,
    db: Session = Depends(get_db),
    id: str
) -> Any:
