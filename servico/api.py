...
def read_categoria_by_id(
    ...
    if not categoria:
        raise HTTPException(
            status_code=404,
            detail="Esse categoria não existe no sistema",
        )
    return categoria
...
def update_categoria(
    *,
    ...
    categoria = cruds.categoria.update(
        db, db_obj=categoria, obj_in=categoria_in)
    return categoria


@router_categoria.delete("/{id}", response_model=schemas.Categoria, dependencies=[
                         Depends(security.has_permission("servico.delete_categoria"))])
...
@router_tiposervico.get(
    "/",
    response_model=List[schemas.TipoServico],
    dependencies=[Depends(security.has_permission("servico.view_tiposervico"))]
)
def read_tiposervicos(
    db: Session = Depends(get_db),
    skip: int = 0,
    limit: int = 25
) -> Any:
...
@router_tiposervico.post("/", response_model=schemas.TipoServico, dependencies=[
                         Depends(security.has_permission("servico.add_tiposervico"))])
def create_tiposervico(
    *,
    db: Session = Depends(get_db),
    tiposervico_in: schemas.TipoServicoCreate,
) -> Any:
    """
...
@router_tiposervico.get(
    "/{tiposervico_id}",
    response_model=schemas.TipoServico,
    dependencies=[
        Depends(
            security.has_permission("servico.view_tiposervico"))])
def read_tiposervico_by_id(
    tiposervico_id: str,
    db: Session = Depends(get_db)
) -> Any:
    """
    ...
    if not tiposervico:
        raise HTTPException(
            status_code=404,
            detail="Esse tiposervico não existe no sistema",
        )
    return tiposervico


@router_tiposervico.put(
    "/{tiposervico_id}",
    response_model=schemas.TipoServico,
    dependencies=[
        Depends(
            security.has_permission("servico.change_tiposervico"))])
def update_tiposervico(
    *,
    db: Session = Depends(get_db),
    tiposervico_id: str,
    tiposervico_in: schemas.TipoServicoUpdate
) -> Any:
    """
    ...
    tiposervico = cruds.tiposervico.update(
        db, db_obj=tiposervico, obj_in=tiposervico_in)
    return tiposervico


@router_tiposervico.delete(
    "/{id}",
    response_model=schemas.TipoServico,
    dependencies=[
        Depends(
            security.has_permission("servico.delete_tiposervico"))])
def delete_tiposervico(
    *,
    db: Session = Depends(get_db),
    id: str
) -> Any:
...
def create_servico(
    *,
    db: Session = Depends(get_db),
    servico_in: schemas.ServicoCreate,
) -> Any:
    """
    Create new servico.
    """
...
def read_servico_by_id(
    ...
    if not servico:
        raise HTTPException(
            status_code=404,
            detail="Esse servico não existe no sistema",
        )
    return servico
...
@router_servico.delete("/{id}", response_model=schemas.Servico, dependencies=[
                       Depends(security.has_permission("servico.delete_servico"))])
def delete_servico(
    *,
    db: Session = Depends(get_db),
    id: str
